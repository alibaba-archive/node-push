...
class Mailgun extends EventEmitter {
  ...
  send (data, callback) {
    if (!this.options.domain) {
      return this.emit('error', new Error('domain is required'))
    }
    const api = `https://${this.options.apiUrl}/v2/${
      this.options.domain
    }/messages`
  ...
  updateSubscribe (listAddress, data = {}, callback) {
    const api = `https://${
      this.options.apiUrl
    }/v2/lists/${listAddress}/members/${data.address}`
    data.method = 'PUT'
    return this.request(api, data, callback)
  }
  ...
  request (api, data, callback) {
    ...
    if (!this.options.apiKey) {
      return callback(new Error('apiKey is required'))
    }
      ...
      function (err, body, resp) {
        try {
          ...
          if (resp.statusCode !== 200) {
            if (!err) {
              err = new Error(body.error || body.message || body)
            }
          }
        } catch (e) {
          if (!err) {
