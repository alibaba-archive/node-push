...
class ApplePushNotification extends EventEmitter {
  ...
  getInvalidDevices(callback) {
    ...
    const feedback = new apns.Feedback(connectionOptions);
    feedback.once('error', callback);
    return feedback.on('feedback', rows => callback(null, rows.map(row => row.device.toString())));
  }
  ...
  send(data = {}) {
    ...
    const myDevice = new apns.Device(data.deviceToken);
    ...
    } else { note.sound = this.options.sound; }
    note.alert = data.alert;
    if (data.extra) { note.payload = data.extra; }

    if (data.contentAvailable) { note.contentAvailable = 1; }
...
const exports = (module.exports = applePN);
