  ...
  send(data = {}) {
    ...
    , function(err, body, resp) {
      ...
      if (body.results && Array.isArray(body.results)) {
        return body.results.map(function(result, i) {
          if (!data.to && !data.registration_ids) { return; }
          const reg_id = data.to || (data.registration_ids != null ? data.registration_ids[i] : undefined);
          if (result.error) { return self.emit('invalid device', reg_id); }
        });
      }
    });
  }
...
const gcm = new GCM;
gcm.GCM = GCM;
const exports = (module.exports = gcm);
